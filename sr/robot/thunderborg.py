...
B_I2C_GPIO_ANALOGUE_START_L = 13
...
class BlackJackBoardGPIO(object):
    ...
    def pin_mode(self, pin, mode):
        ...
        if pin == 2 and mode == INPUT_ANALOGUE:
            raise IndexError("Pin 2 is NOT an analogue input! Use something else!")
    ...
    def analogue_read(self, pin):
        ...
        command = B_I2C_GPIO_ANALOGUE_START_L
        if pin == 3:
            command = B_I2C_GPIO_ANALOGUE_START_L + 2
        if pin == 4:
            command = B_I2C_GPIO_ANALOGUE_START_L + 4
